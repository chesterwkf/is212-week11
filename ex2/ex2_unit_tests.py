import unittest
from ex2_smelly import Circle, Rectangle, Triangle  # Adjust if the class is in another file

class TestShape(unittest.TestCase):

    def test_calculate_area_circle(self):
        # Test for a circle with radius 5
        result = Circle().calculate_area('circle', radius=5)
    ...
    def test_calculate_area_rectangle(self):
        ...
        result = Rectangle().calculate_area('rectangle', length=4, width=6)
        self.assertEqual(result, 24)

    def test_calculate_area_triangle(self):
        # Test for a triangle with base 10 and height 8
        result = Triangle().calculate_area('triangle', base=10, height=8)
